...
import argparse
import json
from preprocessing import DataPreprocessingMid
from preprocessing import DataPreprocessingReady
from run import Run
...
if __name__ == '__main__':
    ...
    if args.process_data_ready:
        for ratio in [[0.8, 0.2], [0.5, 0.5], [0.2, 0.8]]:
            for task in ['1', '2', '3']:
                DataPreprocessingReady(config['root'], config[
                    'src_tgt_pairs'], task, ratio).main()
    print('task:{}; model:{}; ratio:{}; epoch:{}; lr:{}; gpu:{};'.
        format(args.task, args.base_model, args.ratio, args.epoch, args.lr,
        args.gpu))
