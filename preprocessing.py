...
class DataPreprocessingMid:
    ...
    def main(self):
        ...
        re = []
        with gzip.open(self.root + 'raw/reviews_' + self.dealing +
            '_5.json.gz', 'rb') as f:
...
class DataPreprocessingReady:
    ...
    def mapper(self, src, tgt):
        print('Source inters: {}, uid: {}, iid: {}.'.format(len(src), len(
            set(src.uid)), len(set(src.iid))))
        print('Target inters: {}, uid: {}, iid: {}.'.format(len(tgt), len(
            set(tgt.uid)), len(set(tgt.iid))))
        ...
        iid_dict_tgt = dict(zip(set(tgt.iid), range(len(set(src.iid)), len(
            set(src.iid)) + len(set(tgt.iid)))))
    ...
    def split(self, src, tgt):
        ...
        test_users = set(random.sample(co_users, round(self.ratio[1] * len(
            co_users))))
        train_src = src
    ...
    def save(self, train_src, train_tgt, train_meta, test):
        output_root = self.root + 'ready/_' + str(int(self.ratio[0] * 10)
            ) + '_' + str(int(self.ratio[1] * 10)
            ) + '/tgt_' + self.tgt + '_src_' + self.src
        if not os.path.exists(output_root):
            os.makedirs(output_root)
        print(output_root)
        train_src.to_csv(output_root + '/train_src.csv', sep=',', header=\
            None, index=False)
        train_tgt.to_csv(output_root + '/train_tgt.csv', sep=',', header=\
            None, index=False)
        train_meta.to_csv(output_root + '/train_meta.csv', sep=',', header=\
            None, index=False)
        test.to_csv(output_root + '/test.csv', sep=',', header=None, index=\
            False)
